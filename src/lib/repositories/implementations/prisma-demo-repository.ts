...
export class PrismaDemoRepository implements IDemoRepository {
  ...
  async findAll(): Promise<DemoDomain[]> {
    ...
    if (!this.prisma.demo) {
      console.warn('[PrismaDemoRepository] Demo model not found in Prisma client. Please run: npx prisma generate')
      return []
    }
    try {
      ...
      if (error?.code === 'P2021' || error?.message?.includes('does not exist')) {
        console.warn('[PrismaDemoRepository] Demo table does not exist. Please run: npx prisma migrate dev')
        return []
      }
      throw error
    }
