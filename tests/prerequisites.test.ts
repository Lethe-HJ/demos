...
describe('prerequisites.sh 主流程', () => {
  ...
  beforeAll(async () => {
    ...
    cpSync(join(projectRoot, 'prisma'), join(tempDir, 'prisma'), {
      recursive: true
    })
    cpSync(
      join(projectRoot, 'prerequisites.sh'),
      join(tempDir, 'prerequisites.sh')
    )
